...
# Tweet time range of original Trump tweets dataset
# 2009-05-04 18:54:00
# 2017-08-10, 10:54:00
trump_tweets %>% 
  arrange(created_at)
...
## -- Setup  Twitter API using rtweet to extract tweets -- ##
...
trump_all <- trump_all %>% 
  mutate(all_caps_count = sapply(all_caps, length))
...
trump_all <- trump_all %>% 
  mutate(uppercase_first_count = sapply(uppercase_first, length))
...
final_test_trump <- trump_all %>% setdiff(train_trump)
...
saveRDS(final_test_trump, "data/processed/final_test_trump.rds")
write_as_csv(final_test_trump, "data/processed/final_test_trump.csv")
